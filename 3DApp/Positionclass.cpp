...
//Mouse
void PositionClass::MouseMovement(int mouseX, int mouseY)
{	
	//Mouse logic to move the camera	
	if (mouseX < 0)
	{
	...
	if (mouseY > 0)
	{
		m_lookUpSpeed -= m_frameTime * 125.0f;
	}

	if (mouseY < 0)
	{
		m_lookDownSpeed -= m_frameTime * 125.0f;
	}
