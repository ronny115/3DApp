...
{
	// Store the old postion and update the location of the mouse cursor based on the change of the mouse location during the frame.
	...
	// Mouse movement.
	if ((m_mouseX - m_oldMouseX) > 0) { m_mouseX = 1; } //Moved Right
	if ((m_mouseX - m_oldMouseX) < 0) { m_mouseX = -1;} //Moved Left
	if ((m_mouseX - m_oldMouseX) == 0){ m_mouseX = 0; } //Stoped
	
	if ((m_mouseY - m_oldMouseY) > 0) { m_mouseY = 1; } //Moved Up
	if ((m_mouseY - m_oldMouseY) < 0) { m_mouseY = -1;} //Moved Down
	if ((m_mouseY - m_oldMouseY) == 0){ m_mouseY = 0; } //Stoped
...
int InputClass::GetMouseX(int& mouseX)
{
	mouseX = m_mouseX;
	return mouseX;
}

int InputClass::GetMouseY(int& mouseY)
{
	mouseY = m_mouseY;
	return mouseY;
}
